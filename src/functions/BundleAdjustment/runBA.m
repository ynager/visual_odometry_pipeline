...
if with_pattern
    ...
    %THOMAS: I GUESS THIS NUM_OVS DOES ONLY WORK AS NBR LANDMARKS IS CONST
    ...
    %THOMAS: every error therm has its possible influence on 9 terms, all
    %other zeros. pattern describes which entries in Jacobi are NOT zero
    ...
    for frame_i = 1:num_frames
        %THOMAS: get ki in pdf, this might be adapted!
        ...
        for kp_i = 1:numel(landmark_indices)
            %THOMAS: run through the error corresponding to the kp_i
            %landmark
        ...
        %THOMAS: this might need to be adapted!
